...
package com.example.wirtualnatalia.cloudanchor;
...
import com.example.wirtualnatalia.common.helpers.CameraPermissionHelper;
import com.example.wirtualnatalia.common.helpers.DisplayRotationHelper;
import com.example.wirtualnatalia.common.helpers.FullScreenHelper;
import com.example.wirtualnatalia.common.helpers.SnackbarHelper;
import com.example.wirtualnatalia.common.helpers.TrackingStateHelper;
import com.example.wirtualnatalia.common.rendering.BackgroundRenderer;
...
//  private final ObjectRenderer virtualObjectShadow = new ObjectRenderer();
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    addCardButton = findViewById(R.id.add_card_btn);
    addCardButton.setOnClickListener((view) -> onAddCardButton());
    ...
    for (int k = 0; k < Board.MAX_CARDS; k++){
      virtualCards.add(new ObjectRenderer());
    }

    httpClient = (HTTPClient) getIntent().getSerializableExtra("HTTPClient");  // can be null
    Log.i(TAG, "address: " + httpClient.getHost().getHostAddress());
  }
  ...
  public void onSurfaceCreated(GL10 gl, EGLConfig config) {
    ...
    try {
      ...
      // create virtual cards placeholders
      for (ObjectRenderer vc: virtualCards){
        vc.createOnGlThread(this, "models/rect.obj", "models/rect.png");
        vc.setMaterialProperties(0.0f, 2.0f, 0.5f, 6.0f);
      }
      ...
      synchronized (anchorLock) {
        if (anchor != null && anchor.getTrackingState() == TrackingState.TRACKING) {
          ...
          for(Card card: board.getCards()){
            anchorPose.toMatrix(card.getAnchorMatrix(), 0);
            Matrix.translateM(card.getAnchorMatrix(), 0, 0.07f * k, 0, 0);
            Matrix.rotateM(card.getAnchorMatrix(), 0, 112, 1f, 0f, 0f);
            Matrix.rotateM(card.getAnchorMatrix(), 0, -35, 0f, 1f, 0f);
//            translatePose(anchorPose, k).toMatrix(card.getAnchorMatrix(), 0);
            k += 1;
          }
...
//    board.addCard(card);
  ...
  public Pose translatePose(Pose pose, int cardNum){
    final float TRANSLATION = 0.06f;  // ~5cm move
    Log.i(TAG, "Pose before: " + pose.toString());
    pose = Pose.makeTranslation(TRANSLATION * cardNum, 0 , 0).compose(pose);
    Log.i(TAG, "After before: " + pose.toString());
    return pose;
  }
